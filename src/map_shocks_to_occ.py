...
grouped_df = (
    merged_df.groupby(["region", "time", "OCC_CODE"]).agg(total_jobs=("jobs_by_occupation", "sum")).reset_index()
)
...
national_totals = merged_df.groupby(["time", "OCC_CODE"]).agg(total_jobs=("jobs_by_occupation", "sum")).reset_index()
...
# TODO: Some regions are split in two; Maria just divided the population in two
dict_lf_region = {  # LF = labour force
    "Alberta.a": 2.59e6,
...
def load_data(region):
    # Define file paths
    adj_matrix_path = f"../data/networks/occ_mobility_fromusa.csv"
    df_path = f"../data/copper/scenario_{region.replace('.', '_')}.csv"  # Choose the region
