...
def generate_monthly_range(start_year: int, end_year: int):
    return pd.date_range(start=f"{start_year}-01-01", end=f"{end_year}-12-01", freq="MS")


def distribute_jobs(df: pd.DataFrame, years: list[int] = YEARS, bulk_years: int = 2):
    data = []
    national_data = []

    cumulative_jobs = {"CPY_negative": 0, "Op_positive": 0, "Op_negative": 0}
        ...
        for period_start in years:
            ...
            for job_column, job_type in job_columns:
                ...
                if job_type == "CPY_positive_bulk":
                    ...
                    cumulative_jobs_acc = cumulative_jobs[cumulative_key] + np.cumsum(
                        [job_per_month_acc] * months_in_period
                    )
    ...
    national_grouped = df_expanded.groupby(
        ["variable", "model", "scenario", "time", "unit", "job_type"], as_index=False
    ).sum()
...
]  # This is what the people are reading,
...
jobs_types_final = [
    "CPY_positive_bulk",
    "CPY_negative_accumulated",
    "Op_positive_accumulated",
    "Op_negative_accumulated",
]
...
def distribute_jobs(df, start_year=2021, end_year=2050, bulk_years=2):
    ...
    cumulative_jobs = {"CPY_positive": 0, "CPY_negative": 0, "Op_positive": 0, "Op_negative": 0}
...
def distribute_jobs(df, start_year=2021, end_year=2050, bulk_years=2):
    ...
    national_grouped = national_df.groupby(
        ["variable", "model", "scenario", "time", "unit", "job_type"], as_index=False
    ).sum()
    national_grouped["region"] = "National"
...
def make_accumulate_jobs(df, building_time=12):
    ...
    for index, row in df.iterrows():
        ...
        if row["CPY (Direct)_linear_positive"] > 0:
            temp_df.loc[: building_time - 1, "CPY (Direct)_linear_positive"] = (
                row["CPY (Direct)_linear_positive"] / building_time
            )
        ...
        if cumulative_op_positive[row["region"]] > 0:
            temp_df.loc[building_time:, "Op FTE (Direct)_linear_positive"] = cumulative_op_positive[row["region"]] / (
                months_in_period - building_time
            )
...
def plot_jobs_time_series(df, regions):
    ...
    for region in regions:
        ...
        axs[0, 0].plot(
            region_data_grouped["time"],
            region_data_grouped["CPY (Direct)_linear_positive"],
            label=f"{region} CPY Created",
        )
        axs[1, 0].plot(
            region_data_grouped["time"],
            region_data_grouped["CPY (Direct)_linear_negative"],
            label=f"{region} CPY Destroyed",
        )

        axs[0, 1].plot(
            region_data_grouped["time"],
            region_data_grouped["Op FTE (Direct)_linear_positive"],
            label=f"{region} Op Created",
        )
        axs[1, 1].plot(
            region_data_grouped["time"],
            region_data_grouped["Op FTE (Direct)_linear_negative"],
            label=f"{region} Op Destroyed",
        )
...
tools.display_dataframe_to_user(name="Accumulated Jobs Data", dataframe=accumulated_jobs)


# Call the function to create the accumulated jobs DataFrame
accumulated_jobs = accumulate_jobs(df, building_time=12)

accumulated_jobs[(accumulated_jobs["region"] == "National") & (accumulated_jobs["time"] == pd.Timestamp("2025-01-31"))]
...
monthly_jobs[(monthly_jobs["region"] == "National") & (monthly_jobs["time"] == pd.Timestamp("2025-01-31"))]
...
aggregated_df = (
    df.groupby(["region", "time"]).agg({"CPY (Direct)_linear": "sum", "Op FTE (Direct)_linear": "sum"}).reset_index()
)

# Aggregating across all regions and time
total_aggregation = aggregated_df.agg({"CPY (Direct)_linear": "sum", "Op FTE (Direct)_linear": "sum"}).reset_index()
...
print(
    df[
        [
            "CPY (Direct)_linear",
            "CPY (Direct)_linear_positive",
            "CPY (Direct)_linear_negative",
            "Op FTE (Direct)_linear",
            "Op FTE (Direct)_linear_positive",
            "Op FTE (Direct)_linear_negative",
        ]
    ]
)
...
for region in unique_regions:
    ...
    region_aggregate_by_year = (
        region_df.groupby("time")
        .agg(
            {
                "CPY (Direct)_linear_positive": "sum",
                "CPY (Direct)_linear_negative": "sum",
...
bc_aggregate = bc_df.groupby("time").agg({"CPY (Direct)_linear": "sum", "Op FTE (Direct)_linear": "sum"}).reset_index()

# Aggregate across all regions and time
total_aggregate = (
    df.groupby("region").agg({"CPY (Direct)_linear": "sum", "Op FTE (Direct)_linear": "sum"}).reset_index()
)
