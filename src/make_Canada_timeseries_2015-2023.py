...
# filters for data
gdp_filters = {"North American Industry Classification System (NAICS)": "All industries [T001]"}
unemp_filters = {
...
df_gdp_hp["GBP_BB3MA_minmax_interact"] = df_gdp_hp.apply(
    lambda row: (
        row["GDP_Cycle12"]
        if row["GDP_Cycle12"] < 1
        else (row["BBarometer_Cycle_3MA"] if row["BBarometer_Cycle_3MA"] > 1 else 1)
    ),
    axis=1,
)
...
plot_time_series(
    ...
    df_gdp_hp.index,
    df_gdp_hp["Log_VALUE"],
    "Log GDP (seasonaly adjusted) lambda = 129600",
    "log GDP",
)
...
plot_time_series(
    ...
    df_gdp_hp.index,
    df_gdp_hp["Log_VALUE"],
    "Log GDP (seasonaly adjusted) lambda = 14400",
    "log GDP",
)
# Plot Normalized GDP Growth Rate
...
plot_time_series(
    ...
    1,
    df_gdp["REF_DATE"],
    df_gdp["Fluctuations_norm"],
    "GDP growth normalized and standardized",
    "Fluctuations",
)
# Plot Job Vacancy Rate
plot_time_series(axes, 2, df_vac["REF_DATE"], df_vac["VALUE"], "Job Vacancy Rate (seasonaly adjusted)", "Rate (%)")
...
plot_time_series(
    ...
    df_unemp["REF_DATE"],
    df_unemp["VALUE"],
    "Unemployment Rate (not seasonaly adjusted)",
    "Rate (%)",
)
# Plot Normalized GDP Growth Rate
...
plot_time_series(
    ...
    1,
    df_gdp["REF_DATE"],
    df_gdp["Normalized_Growth_Rate"],
    "Normalized GDP Growth Rate",
    "Normalized Rate",
)
# Plot Job Vacancy Rate
plot_time_series(axes, 2, df_vac["REF_DATE"], df_vac["VALUE"], "Job Vacancy Rate (seasonaly adjusted)", "Rate (%)")
...
df_vac = df_vac[
    (df_vac["North American Industry Classification System (NAICS)"] == "Total, all industries")
...
gdp_normalized = (df_gdp_sync["GDP_Growth_Rate"] - df_gdp_sync["GDP_Growth_Rate"].mean()) / df_gdp_sync[
    "GDP_Growth_Rate"
].std()
